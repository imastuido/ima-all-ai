...
def build_image_model_candidates(product_tree: list, spec: TaskSpec) -> tuple[ModelCandidate, ...]:
    rows = list_all_models(product_tree, task_type=spec.task_type)
    return tuple(
        ModelCandidate(name=row["name"], model_id=row["model_id"], version_id=row["version_id"])
        for row in rows
    )


def build_image_model_binding(model_params: dict) -> ModelBinding:
