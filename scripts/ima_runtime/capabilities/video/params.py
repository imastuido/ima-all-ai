...
def normalize_video_binding(spec: TaskSpec, model_params: dict) -> tuple[dict, dict]:
    ...
    if "generate_audio" in extra_params and "audio" not in extra_params and "audio" in form_keys:
        extra_params["audio"] = bool(extra_params["generate_audio"])
        extra_params.pop("generate_audio", None)
    ...
    allowed = form_keys | rule_keys | virtual_ui_keys | VIDEO_ALLOWED_PARAMS
