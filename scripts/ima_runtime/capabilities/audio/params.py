...
def _resolve_virtual_overrides(extra_params: dict | None, model_params: dict) -> dict:
    ...
    for group in virtual_groups:
        ...
        for ui_field in ui_fields:
            if ui_field != target_param:
                resolved.pop(ui_field, None)
        resolved[target_param] = matched_target_value
    ...
    for ui_field, mapping in virtual_mappings.items():
        ...
        resolved[target_param] = mapped_value
...
def normalize_audio_binding(spec: TaskSpec, model_params: dict) -> tuple[dict, dict]:
    ...
    allowed = form_keys | rule_keys | virtual_ui_keys | AUDIO_ALLOWED_PARAMS
...
def build_audio_model_params(binding: ModelBinding) -> dict:
    ...
    return {
        ...
        "form_fields": list(metadata.get("form_fields") or sorted((metadata.get("form_params") or binding.resolved_params).keys())),
